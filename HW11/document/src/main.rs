...
fn main() {
    ...
    documents.sort_by(|a, b| b.1.len().cmp(&a.1.len())); // Sort by paragraphs

    println!("HTML Table - Paragraph Count:");
    ...
    for (filename,docu) in &documents {
        html.push_str("\n\t<tr>");
        html.push_str(&format!("\n\t\t<td>{}</td>\n\t\t<td>{}</td>", filename, docu.len()));
        html.push_str("\n\t</tr>");
    }
    html.push_str("</table>");
    println!("{}",html); // use cargo run --bin document file1.txt file2.txt file3.txt > output.html
